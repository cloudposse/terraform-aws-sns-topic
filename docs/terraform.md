...
| <a name="input_fifo_queue"></a> [fifo\_queue](#input\_fifo\_queue) | Whether or not to create a FIFO (first-in-first-out) queue | `bool` | `false` | no |
...
| <a name="input_redrive_policy"></a> [redrive\_policy](#input\_redrive\_policy) | The SNS redrive policy as JSON. This overrides the deadLetterTargetArn passed by the module. | `string` | `null` | no |
