locals {
  ...
  sns_topic_policy_enabled = local.enabled && (length(var.sns_topic_policy_json) > 0 || length(var.allowed_aws_services_for_sns_published) > 0 || length(var.allowed_iam_arns_for_sns_publish) > 0)
}
